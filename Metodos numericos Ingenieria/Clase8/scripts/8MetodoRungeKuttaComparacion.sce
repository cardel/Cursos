...
function [result,points]=metodoRungeKutta(stepsize,y0, t0, tf, eqf)
    ...
    for i=2:1:length(points)
       ...
       k2 = eqf(y0+k1*stepsize,points(i-1)+stepsize)
       result(i) = ya+(0.5*k1+0.5*k2)*stepsize
    end
endfunction
...
stepsize = 1
[resultsA, pointsA] = metodoRungeKutta(stepsize,2,0,10,eqdiff)
[resultsB, pointsB] = metodoRungeKuttaMedio(stepsize,2,0,10,eqdiff)
[resultsC, pointsC] = metodoRungeKuttaRalston(stepsize,2,0,10,eqdiff)
...
plot(pointsB,resultsB,"color","green","marker","*")
plot(pointsC,resultsC,"color","cyan","marker",">")
legend(['Método analitico';'Método de Runge-Kutta a = 1/2';'Método de Runge-Kutta a = 1';'Método de Runge-Kutta a = 2/3'],[-1])
xgrid()
