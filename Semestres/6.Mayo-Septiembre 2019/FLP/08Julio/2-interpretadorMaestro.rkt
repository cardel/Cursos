...
(define grammar-simple-interpreter
  '((program (expression) a-program)
    ...
    (primitive ("first") first-prim)
    (primitive ("rest") rest-prim)
    (primitive ("empty?") empty-prim)
    ))
...
(define apply-primitive
  (lambda (prim args)
    (cases primitive prim
      ...
      (first-prim () (car (car args)))
      (rest-prim () (cdr (car args)))
      (empty-prim () (if (null? (car args)) 10 0))
      )))
