...
#|
Secuenciación
<expresion> ::= "begin" <expresion> (";" <expresion>)*
                 begin-exp(exp lexp)
|#
...
(define especificacion-gramatical
  '(
    ...
    ;;Set
    (expresion ("set" texto "=" expresion) set-exp)
    ;;Primitivas
    (primitiva ("+") sum-prim)
    (primitiva ("-") res-prim)
    (primitiva ("*") mul-prim)
...
;;Este señor va a retornar una referencia (implicita)
    ...
    (cases expresion exp
      ...
      (local-exp (lid lexp exp)
                 (let
                   ...
                   (evaluar-expresion
                    exp
                    (ambiente-extendido lid (list->vector lexp1) amb))))
      ;;Procedimientos
      (proc-exp (largs body)
                 ...
                 (if
                  ...
                  (cases procval proc
                    (clausura
                     (lids body env-old)
                     (if
                      (= (length lids) (length lrands))
                      (evaluar-expresion body (ambiente-extendido lids (list->vector lrands) env-old))
                      (eopl:error "La función espera " (length lids) " y usted mandó " (length lrands)))
                     ))
