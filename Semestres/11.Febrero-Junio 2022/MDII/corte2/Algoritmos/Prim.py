...
def prim(MR,etiquetas):
    ...
    for k in range(MR.shape[0]-2):
        ...
        for i in range(MR.shape[0]):
            for j in range(MR.shape[1]):
                #Escojer la menor arista global al arbol
                #Sin que sea ciclo
                u = etiquetas[i]
                v = etiquetas[j]
                peso = MR[i][j]
                if not(u in nodos_arbol and v in nodos_arbol) and MR[i][j]>0:
...
lista_aristas = prim(grafo,nodos)

Grafo = nx.Graph()
Grafo = Grafo.to_directed()
Grafo.add_nodes_from(nodos)
Grafo.add_edges_from(lista_aristas)
nx.draw(Grafo,with_labels=True,node_size=1000,node_color="green")
plt.show()
