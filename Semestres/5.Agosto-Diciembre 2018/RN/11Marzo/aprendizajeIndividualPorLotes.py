...
def entrenamientoPorLotes(entradas, M, factorEntre, maxIT, P):
	"""
		entradas: vector 2D
	...
	for i in range(0,maxIT):
		...
		diferencias = 0
		for k in range(0,entrada.shape[0]):
			hI,rI,zI = redNeuronal(entrada[k], pesos)
			
			if rI == r:
				for j in range(0, pesos.shape[1]):
					diferencias += entrada[k][j]-pesos[r][j]
		
		for j in range(0, pesos.shape[1]):
			pesos[r][j] = pesos[r][j] + factorEntre*(diferencias)
...
pesos = entrenamientoPorLotes(entrada, M, factorEntre, maxIT, P)
